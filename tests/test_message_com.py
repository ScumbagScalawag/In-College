...
def test_send_message(setup_users, monkeypatch):
    ...
    message = Message(
        sender.username, recipient.username, "Test Subject", "Hello! This is a test message."
    )
    ...
    assert any(
        msg.message == "Hello! This is a test message." for msg in recipient_after.incomingMessages
    ), "Message was not found in recipient's inbox"
...
def test_send_message_non_plus_to_friend(setup_users, monkeypatch, capfd):
    ...
    # JSON needs to be reread for reasons ???? user_db.loadUsers() ????
    with open(JSON_USERS_FP, "r") as file:
        data_after_test = json.load(file)
    #?????
    recipient_data = next(
        (user for user in data_after_test["userlist"] if user["username"] == "user3")
    )
    last_message = recipient_data["incomingMessages"][-1]
    assert last_message["message"].strip() == "Test Message\nSEND"
...
def test_respond_back(setup_users, monkeypatch, capfd):
    ...
    inputs = iter(["2", "1", "1", "y", "subject:sendback", "trying to send body", "SEND"])
    ...
    with open(JSON_USERS_FP, "r") as file:
        data_after_test = json.load(file)
    recipient_data = next(
        (user for user in data_after_test["userlist"] if user["username"] == "user2")
    )
    last_message = recipient_data["incomingMessages"][-1]
    assert last_message["message"].strip() == "trying to send body\nSEND"
